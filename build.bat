...
set MINGW_PATH=%CD%\pico-toolchain\mingw32-w64-i686\bin
set ELF2UF2_PATH=%CD%\pico-toolchain\elf2uf2-mingw32-i686
set PIOASM_PATH=%CD%\pico-toolchain\pioasm-mingw32-i686
set DOXYGEN_PATH=%CD%\pico-toolchain\doxygen-1.9.1-windows
...
set PATH=%PICO_TOOLCHAIN_PATH%;%CMAKE_PATH%;%MAKE_PATH%;%PYTHON_PATH%;%DOXYGEN_PATH%;%PIOASM_PATH%;%ELF2UF2_PATH%;%MINGW_PATH%

mkdir %PWD%\%BUILD%
cd %PWD%\%BUILD%
cmake %CMAKE_FLAGS%
cd %PWD%\%BUILD%
make blink -i VERBOSE=1
cd %PWD%\%BUILD%\pio
make -i VERBOSE=1
cd %PWD%\%BUILD%
cmake %CMAKE_FLAGS%
